...
st.set_page_config(page_title="CyberAudits - Escáner Perimetral", page_icon="🛡️", layout="centered")
...
def scan_ports(hostname):
    common_ports = {
        ...
        3306: "MySQL", 8080: "HTTP-Proxy", 8443: "HTTPS-Panel"
    }
    open_ports = []
    ...
    for p in open_ports:
        if p['port'] in [21, 3306, 8080, 8443]:
            ...
            findings.append({
                ...
                'severity': 'MEDIO',
                'badge': 'badge-medium',
                'exec_title': f"Servicio Administrativo / Base de Datos Expuesto en Puerto {p['port']}",
                'desc': f"El puerto {p['port']} ({p['service']}) se encuentra accesible de forma directa desde internet sin restricciones perimetrales visibles.",
                'impact': 'Invita a atacantes a realizar ataques de fuerza bruta para adivinar credenciales y lograr control total de la plataforma.',
                'fix': f"Restringir el acceso al puerto {p['port']} mediante un Firewall de Red o Grupos de Seguridad."
            })
    ...
    try:
        ...
        if 'Strict-Transport-Security' in headers:
            ...
            findings.append({
                ...
                'exec_title': 'Ausencia de Encriptación Forzada (Riesgo de Intercepción)',
                'desc': 'La cabecera HSTS no está configurada en la respuesta del servidor web.',
                'impact': 'Un atacante en una red Wi-Fi pública puede interceptar la conexión y robar contraseñas o tokens en tiempo real.',
                'fix': 'Configurar la cabecera: Strict-Transport-Security: max-age=31536000; includeSubDomains; preload.'
            })
        ...
        if 'Server' in headers:
            ...
            findings.append({
                ...
                'desc': f"La cabecera HTTP expone el software y versión exacta: {headers.get('Server')}",
                'impact': 'Facilita que actores maliciosos busquen vulnerabilidades públicas asociadas a esa versión exacta de software.',
                'fix': 'Ocultar o enmascarar la firma del servidor en la configuración global.'
            })
        else:
            stats['Seguras'] += 1
        ...
        if 'X-Frame-Options' in headers or 'Content-Security-Policy' in headers:
            ...
            stats['Bajas'] += 1
            findings.append({
                'vector': 'Protección contra Clickjacking Ausente (X-Frame-Options)',
                'severity': 'BAJO',
                'badge': 'badge-low',
                'exec_title': 'Riesgo de Secuestro de Clics (Clickjacking)',
                'desc': 'El sitio web no emite directivas para evitar su carga dentro de marcos externos.',
                'impact': 'Un sitio malicioso externo puede cargar tu web de forma invisible bajo un botón trampa para engañar al usuario.',
                'fix': 'Añadir la cabecera de seguridad X-Frame-Options: DENY o SAMEORIGIN.'
            })
        ...
        if cookies:
            for cookie in cookies:
                has_httponly = False
                if hasattr(cookie, '_rest') and any(attr.lower() == 'httponly' for attr in cookie._rest):
                    has_httponly = True
                ...
                if not cookie.secure or not has_httponly:
                    ...
                    findings.append({
                        ...
                        'severity': 'MEDIO',
                        'badge': 'badge-medium',
                        'exec_title': f'Vulnerabilidad en Cookie de Sesión ({cookie.name})',
                        'desc': f"La cookie '{cookie.name}' carece de los atributos de protección Secure u HttpOnly.",
                        'impact': 'Scripts maliciosos (XSS) pueden robar la sesión activa del usuario y suplantar su identidad.',
                        'fix': 'Emitir cookies con los atributos: Secure; HttpOnly; SameSite=Strict.'
                    })
                else:
                    stats['Seguras'] += 1
        else:
...
def generate_pdf(url, findings, stats, chart_base64, open_ports, hostname, subdomains, output_filename):
    ports_html = "".join([f"<tr><td><code>{p['port']}</code></td><td><strong>{p['service']}</strong> (Abierto)</td></tr>" for p in open_ports]) or "<tr><td colspan='2' style='text-align:center;'>No se detectaron puertos comunes.</td></tr>"
    sub_html = "".join([f"<li><code>{sub}</code></li>" for sub in subdomains]) or "<li>No se encontraron subdominios adicionales.</li>"
    ...
    exec_bullets = "".join([f"<li><strong>{f['exec_title']}</strong>: {f['impact']}</li>" for f in findings])
    ...
    <head>
        <meta charset="UTF-8">
        <style>
            @page {{ size: A4; margin: 10mm 12mm; background-color: #f8fafc; @bottom-right {{ content: "Página " counter(page) " de " counter(pages); font-size: 8pt; color: #64748b; }} }}
            ...
            .chart-container img {{ max-width: 75%; height: auto; }}
    ...
    <body>
        ...
        <div class="card">
            ...
            <ul style="margin:0; padding-left:14px; font-size:8pt;">{exec_bullets}</ul>
        </div>
...
st.write("Analiza la seguridad de cualquier dominio web y obtén métricas de exposición de infraestructura.")
...
if st.button("🚀 Ejecutar Análisis Gratuito"):
    if not target_url or target_url == "https://":
        ...
        with st.spinner("🔍 Analizando superficie perimetral..."):
            ...
            generate_pdf(target_url, findings, stats, chart_b64, open_ports, hostname, subdomains, pdf_filename)
...
if st.session_state.scanned:
    st.success(f"¡Análisis preliminar completado para {st.session_state.hostname}!")
    ...
    if len(st.session_state.findings) > 0:
        st.warning(f"⚠️ ¡Atención! Se han detectado **{len(st.session_state.findings)} riesgos de seguridad** que exponen este dominio a ataques automatizados.")
    else:
        st.info("✅ El dominio muestra una superficie perimetral controlada en los vectores básicos analizados.")

    st.markdown("---")
    st.markdown("### 📥 Descarga el Informe Ejecutivo y Técnico Completo")
    st.markdown("Obtén el documento en PDF listo para gerencia, con gráficos detallados, impacto de negocio y las **guías exactas de remediación paso a paso**.")
    
    st.info("💎 **Precio del Reporte Completo:** $9.00 USD / Equivalente local")
    
    # 2 Columnas: Mercado Pago y PayPal con links reales
    col_mp, col_paypal = st.columns(2)
    
    with col_mp:
        st.markdown("🇦🇷 **Mercado Pago** (Pesos / LatAm)")
        st.markdown("[👉 Pagar con Mercado Pago](https://mpago.li/2RNZnfh)", unsafe_allow_html=True)
        
    with col_paypal:
        st.markdown("🅿️ **PayPal** (Saldo / USD)")
        st.markdown("[👉 Pagar con PayPal](https://www.paypal.me/nielsen1989/9USD)", unsafe_allow_html=True)
    
    st.markdown("")
    st.write("---")
    st.markdown("#### 🔓 ¿Ya realizaste el pago o quieres probar la descarga?")
    
    pago_verificado = st.checkbox("Simular que el pago fue exitoso (Modo de prueba local)")
    
    if pago_verificado:
        if os.path.exists(st.session_state.pdf_filename):
            with open(st.session_state.pdf_filename, "rb") as pdf_file:
                st.download_button(
                    label="📥 Descargar tu PDF Completo Ahora",
                    data=pdf_file,
            ...
            st.error("El archivo PDF no se encuentra disponible. Vuelve a ejecutar el escaneo.")
