...
# ========== UI PRINCIPAL Y NAVEGACIÓN (PANEL LATERAL CLÁSICO) ==========
...
if menu_option == "Escáner Perimetral":
    ...
    st.markdown('<div class="enterprise-banner">🛠️ Gestión de Tickets y Remediación (Consola Unificada)</div>', unsafe_allow_html=True)
    
    try:
        conn = get_db_connection()
        is_pg = "postgres" in st.secrets
        ph = "%s" if is_pg else "?"
        tickets_df = pd.read_sql_query("SELECT id, hostname, finding_vector, severity, status, notes FROM remediation_tasks ORDER BY id DESC", conn)
        conn.close()
    except Exception:
        tickets_df = pd.DataFrame()
    
    if tickets_df.empty:
        st.info("📭 No hay tareas de remediación registradas todavía. Ejecuta un escaneo perimetral para generar tickets automáticamente.")
    else:
        st.info("💡 Edita los estados (`Pendiente`, `En Proceso`, `Solucionado`) y las notas directamente en la tabla y haz clic en **Guardar Todos los Cambios**.")
        
        # Preparar dataframe para el editor unificado
        edit_df = tickets_df.copy()
        ...
        edited_df = st.data_editor(
            ...
            column_config={
                ...
                "hostname": st.column_config.TextColumn("Host", disabled=True),
                "finding_vector": st.column_config.TextColumn("Vulnerabilidad / Vector", disabled=True),
                "severity": st.column_config.TextColumn("Severidad", disabled=True),
                "status": st.column_config.SelectboxColumn("Estado", options=["Pendiente", "En Proceso", "Solucionado"], required=True),
                "notes": st.column_config.TextColumn("Nota de Bitácora", width="large")
            },
            disabled=["id", "hostname", "finding_vector", "severity"],
            num_rows="fixed",
            key="unified_tickets_editor"
        )
        ...
        with col_btn1:
            if st.button("💾 Guardar Todos los Cambios", type="primary", use_container_width=True):
                ...
                for idx, row in edited_df.iterrows():
                    orig = tickets_df.iloc[idx]
                    if row["status"] != orig["status"] or row["notes"] != (orig.get("notes", "") or ""):
                        if update_ticket_status(int(row["id"]), row["status"], row["notes"] if pd.notna(row["notes"]) else ""):
                            cambios += 1
                            
                if cambios > 0:
                    st.success(f"✅ ¡Se actualizaron {cambios} tickets correctamente!")
                    if any(edited_df["status"] == "Solucionado"):
                        st.balloons()
                    st.rerun()
                else:
                    st.info("No se detectaron modificaciones pendientes de guardar.")
        with col_btn2:
            st.caption("🔄 Todos los tickets se gestionan aquí de forma unificada sin que desaparezcan de tu vista.")
